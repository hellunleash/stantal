...
export function agentsSection(version = "0.0.0", provider: ProviderFacts | null = null): string {
  return [
    ...
    "",
    "### Keeping it running without anyone remembering",
    "",
    "Contract drift arrives on the provider's release day, not on your push day, so",
    "nothing you run by hand will be running on the day it matters. Two workflows",
    "cover that, and both ship inside the package at",
    "`node_modules/stantal/templates/`. Copy, do not retype.",
    "",
    "| copy | to | when |",
    "|---|---|---|",
    "| `stantal-watch.yml` | `.github/workflows/` | this project *installs* a contract. Weekly: it reads your dependencies and, when one moves, comments the verdict on an existing Renovate or Dependabot bump, or opens one pull request adding contract tests. |",
    "| `stantal-release-gate.yml` | `.github/workflows/` | this project *publishes* a contract. On every pull request: reads the build against your last published release and fails on a structural break. |",
    "",
    "**Ask before adding either one.** A scheduled workflow spends Actions minutes and",
    "can open pull requests, which is a larger thing to put in somebody's repository",
    "than a config file they can delete in four lines. Say which one you propose and",
    "why, using what the audit found.",
    "",
    "Neither needs an account or an API key. A model key set as a repository secret",
    "turns on Layer 2 in the weekly run and is optional everywhere.",
    "",
